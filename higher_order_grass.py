...
def kgs_greedy(component_adj, summary, k):
    ...
    while supernodeCounter > k:
        ...
        for c in possibleSuperCouple:
            ...
            supernodeS3 = supernode.Supernode(summary.autoincrementId)
...
def kgs_sample_pairs_constant(component_adj, summary, k, num_pair):
    '''
    Given an input hypergraph H(X,E) and integer k, find a summary graph S
    for H with at most k supernodes X (|X| <= k),
    such that the Re(H|S) is minimized.
    '''
...
def kgs_sample_pairs_proportional(component_adj, summary, k, c_const):
    '''
    Given an input hypergraph H(X,E) and integer k, find a summary graph S
    for H with at most k supernodes X (|X| <= k),
    such that the Re(H|S) is minimized.
...
def kgs_linear_check(component_adj, summary, k):
    '''
    Given an input hypergraph H(X,E) and integer k, find a summary graph S
    for H with at most k supernodes X (|X| <= k),
    such that the Re(H|S) is minimized.
...
def kcgs_condense(component_adj, summary, k):
    '''
    Given an input hypergraph H(X,E) and integer k,
    find a summary hypergraph S for H with supernodes
    V such that Re(H|S) is minimized and for every
